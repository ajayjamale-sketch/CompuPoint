...
const FALLBACK_COURSES = [
  { id: "1", title: "Modern JavaScript - ES6+", instructor: "Sarah Chen", duration: "8 hours", image: "https://placehold.co/600x400/7c3aed/white?text=JS", description: "Master modern JavaScript features" },
  { id: "2", title: "React - The Complete Guide", instructor: "David Miller", duration: "24 hours", image: "https://placehold.co/600x400/7c3aed/white?text=React", description: "Build production-ready React apps" },
  { id: "3", title: "Node.js Microservices", instructor: "Alex Rivera", duration: "12 hours", image: "https://placehold.co/600x400/7c3aed/white?text=Node", description: "Scale backend with Node.js" },
  { id: "4", title: "AWS Cloud Practitioner", instructor: "Jordan Lee", duration: "10 hours", image: "https://placehold.co/600x400/7c3aed/white?text=AWS", description: "Prepare for AWS certification" },
  { id: "5", title: "DevOps with Docker & Jenkins", instructor: "Maria Gonzalez", duration: "15 hours", image: "https://placehold.co/600x400/7c3aed/white?text=DevOps", description: "CI/CD pipelines and containers" },
  { id: "6", title: "TypeScript Mastery", instructor: "Tom Wagner", duration: "6 hours", image: "https://placehold.co/600x400/7c3aed/white?text=TS", description: "Type-safe JavaScript" },
];
    ...
    <>
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-[100] p-4">
          <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-2xl max-w-md w-full p-6">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-lg font-bold flex items-center gap-2"><Briefcase className="w-5 h-5 text-purple-500" /> Apply for {showJobApplicationModal.job.title}</h3>
            ...
            <div className="space-y-4">
              ...
              <div className="flex gap-2">
                <button onClick={submitJobApplication} className="flex-1 bg-purple-600 text-white text-sm py-2 rounded-lg flex items-center justify-center gap-1 hover:bg-purple-700 transition"><Send className="w-4 h-4" /> Submit Application</button>
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-[100] p-4">
          <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-2xl max-w-2xl w-full p-6 max-h-[80vh] overflow-y-auto">
            <div className="flex justify-between items-center mb-4 sticky top-0 bg-white dark:bg-slate-900 pt-0 pb-2">
              <h3 className="text-lg font-bold flex items-center gap-2"><BookOpen className="w-5 h-5 text-purple-500" /> Courses to improve "{showCourseRecommendations.skill}"</h3>
                ...
                <div key={course.id} className="flex items-start gap-3 p-3 border rounded-xl hover:bg-slate-50 dark:hover:bg-slate-800">
                  ...
                  <div className="flex-1">
                    ...
                    <button 
                      ...
                      className="text-xs text-purple-600 mt-1 font-medium hover:underline flex items-center gap-1"
                    >
      ...
      {showAddCertModal && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-[100] p-4">
          <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-2xl max-w-md w-full p-6">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-lg font-bold flex items-center gap-2"><Award className="w-5 h-5 text-purple-500" /> Add Certification</h3>
            ...
            <div className="space-y-4">
              ...
              <div className="flex gap-2">
                <button onClick={addNewCertification} className="flex-1 bg-purple-600 text-white text-sm py-2 rounded-lg hover:bg-purple-700 transition">Add Certification</button>
      ...
      {showAddProjectModal && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-[100] p-4">
          <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-2xl max-w-md w-full p-6">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-lg font-bold flex items-center gap-2"><Code2 className="w-5 h-5 text-purple-500" /> Add Portfolio Project</h3>
            ...
            <div className="space-y-4">
              ...
              <div className="flex gap-2">
                <button onClick={addNewProject} className="flex-1 bg-purple-600 text-white text-sm py-2 rounded-lg hover:bg-purple-700 transition">Add Project</button>
        ...
        <div className="space-y-2.5">
          {careerOps.slice(0, 3).map((job: any) => (
            <div key={job.id} className="flex items-center justify-between gap-3 p-3 rounded-xl bg-slate-50 dark:bg-slate-800/50">
              ...
              <div className="text-right"><div className="text-sm font-bold text-purple-600">{job.match}% match</div>{job.applied ? <span className="inline-block text-[11px] bg-green-100 text-green-700 px-2 py-0.5 rounded-full mt-1">Applied</span> : <button onClick={() => onApplyJob(job)} className="bg-purple-600 text-white text-[11px] px-2.5 py-1 rounded-lg mt-1 hover:bg-purple-700 transition">Apply Now</button>}</div>
            </div>
          ))}
        </div>
      </div>
...
function SkillsTab({ skillData, onFindCourses }: any) {
  return (
    <div className="space-y-5">
      <div className="flex justify-between items-center"><h2 className="text-lg font-semibold">Skill Development Roadmap</h2><button className="text-xs text-purple-600">Download Report</button></div>
      <div className="space-y-3">
          ...
          return (
            <div key={skill.skill} className="bg-white dark:bg-slate-900 rounded-xl border p-4 shadow-sm">
              <div className="flex items-center justify-between mb-2 flex-wrap gap-2">
                ...
                <div className="flex items-center gap-3 text-xs"><span>Current: <span className="font-bold text-purple-600">{skill.current}%</span></span><span>Target: <span className="font-bold text-green-600">{skill.target}%</span></span><span className={cn("px-2 py-0.5 rounded-full text-[10px] font-bold", gap > 20 ? "bg-red-50 text-red-600" : gap > 10 ? "bg-yellow-50 text-yellow-600" : "bg-green-50 text-green-600")}>Gap: {gap}%</span></div>
              </div>
              <div className="relative h-2 bg-slate-100 rounded-full"><div className="absolute h-full bg-purple-600 rounded-full" style={{ width: `${skill.current}%` }} /><div className="absolute top-1/2 -translate-y-1/2 w-2.5 h-2.5 border-2 border-green-500 bg-white rounded-full" style={{ left: `${skill.target}%` }} /></div>
              {gap > 0 && <button onClick={() => onFindCourses(skill.skill)} className="text-[11px] text-purple-600 font-medium mt-2 hover:underline flex items-center gap-1">Find courses to close gap <ExternalLink className="w-3 h-3" /></button>}
            </div>
          );
        })}
      </div>
...
function CertsTab({ certifications, onShareCert, onDownloadCert, onAddCert }: any) {
  return (
    <div className="space-y-4">
      <div className="flex justify-between items-center"><h2 className="text-lg font-semibold">Certifications Portfolio</h2><button onClick={onAddCert} className="bg-purple-600 text-white text-xs px-3 py-2 rounded-lg flex items-center gap-1 hover:bg-purple-700 transition"><Plus className="w-3.5 h-3.5" /> Add New</button></div>
      <div className="grid sm:grid-cols-2 gap-4">
        {certifications.map((cert: any) => (
          <div key={cert.id} className="bg-white dark:bg-slate-900 rounded-xl border p-5 shadow-sm">
            <div className="flex items-start gap-3"><div className="w-10 h-10 rounded-xl bg-purple-50 flex items-center justify-center"><Award className="w-5 h-5 text-purple-600" /></div><div className="flex-1"><h3 className="text-sm font-semibold mb-1">{cert.title}</h3><p className="text-xs text-slate-400 mb-2">{cert.issueDate} • {cert.credentialId}</p><div className="flex items-center gap-3"><span className={cn("text-[10px] px-2 py-0.5 rounded-full", cert.status === 'earned' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700')}>{cert.status}</span><button onClick={() => onShareCert(cert.title)} className="text-xs text-purple-600 font-medium hover:underline flex items-center gap-1"><Share2 className="w-3 h-3" /> Share</button><button onClick={() => onDownloadCert(cert.title)} className="text-xs text-purple-600 font-medium hover:underline flex items-center gap-1"><Download className="w-3 h-3" /> Download</button></div></div></div>
          </div>
        ))}
        <div onClick={onAddCert} className="bg-white dark:bg-slate-900 rounded-xl border-dashed border-2 p-5 text-center flex flex-col items-center justify-center min-h-[140px] hover:border-purple-400 cursor-pointer transition"><Award className="w-8 h-8 text-purple-300 mb-2" /><p className="text-sm font-semibold">Add New Certification</p><p className="text-xs text-slate-400">Attempt a new exam</p></div>
      </div>
    </div>
  );
}
...
function PortfolioTab({ portfolio, onAddProject }: any) {
  return (
    <div className="space-y-4">
      <div className="flex justify-between items-center"><h2 className="text-lg font-semibold">Portfolio Projects</h2><button onClick={onAddProject} className="bg-purple-600 text-white text-xs px-3 py-2 rounded-lg flex items-center gap-1 hover:bg-purple-700 transition"><Plus className="w-3.5 h-3.5" /> Add Project</button></div>
      <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {portfolio.map((project: any) => (
          <div key={project.id} className="bg-white dark:bg-slate-900 rounded-xl border p-4 shadow-sm"><div className="w-10 h-10 rounded-xl bg-purple-50 flex items-center justify-center mb-3"><Globe className="w-5 h-5 text-purple-600" /></div><h3 className="text-sm font-semibold mb-1">{project.name}</h3><p className="text-xs text-slate-400 mb-3">{project.tech}</p><div className="flex items-center justify-between"><span className={cn("text-[10px] px-2 py-0.5 rounded-full", project.status === 'live' ? 'bg-green-100 text-green-700' : project.status === 'private' ? 'bg-gray-100 text-gray-700' : 'bg-yellow-100 text-yellow-700')}>{project.status}</span><span className="text-xs text-slate-400">{project.views} views</span></div></div>
        ))}
      </div>
    </div>
  );
...
function CareerTab({ careerOps, onApplyJob }: any) {
  return (
    <div className="space-y-4">
      <div className="flex justify-between items-center"><h2 className="text-lg font-semibold">Career Opportunities</h2><button className="text-xs text-purple-600">Job Alerts</button></div>
      <div className="space-y-3">
        {careerOps.map((job: any) => (
          <div key={job.id} className="bg-white dark:bg-slate-900 rounded-xl border p-4 shadow-sm">
            <div className="flex items-start justify-between gap-4"><div><div className="flex items-center gap-2 mb-1"><h3 className="text-sm font-semibold">{job.title}</h3><span className="text-[10px] bg-gray-100 px-2 py-0.5 rounded-full">{job.type}</span></div><p className="text-xs text-slate-400">{job.company}</p><p className="text-sm font-bold text-green-600 mt-1">{job.salary}</p></div><div className="text-right"><div className="text-xl font-bold text-purple-600">{job.match}%</div><p className="text-[10px] text-slate-400">match score</p>{job.applied ? <span className="inline-block text-[11px] bg-green-100 text-green-700 px-2 py-0.5 rounded-full mt-2">Applied ✓</span> : <button onClick={() => onApplyJob(job)} className="bg-purple-600 text-white text-xs px-3 py-1.5 rounded-lg mt-2 hover:bg-purple-700 transition">Apply Now</button>}</div></div>
          </div>
        ))}
      </div>
    </div>
...
function AnalyticsTab({ user, certifications, careerOps, portfolio, skillData, radarData, salaryData }: any) {
  ...
  // 3. Certification Growth (mock monthly, but scaled to reflect actual count)
  const certsPerMonth = [
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center">
        ...
        <button className="text-xs text-purple-600 hover:underline">Export All Reports</button>
      </div>
      
      {/* === SECTION 1: KPI ROW (New) === */}
      ...
      {/* === SECTION 2: Existing Charts (Radar + Salary) === */}
      <div className="grid md:grid-cols-2 gap-5">
        <div className="bg-white dark:bg-slate-900 rounded-xl border p-4 shadow-sm">
          <h3 className="text-sm font-semibold mb-2 flex items-center gap-2"><Target className="w-4 h-4 text-purple-500" /> Skill Coverage Radar</h3>
          <ResponsiveContainer width="100%" height={260}>
            <RadarChart data={radarData}>
              <PolarGrid stroke="#e2e8f0" />
              <PolarAngleAxis dataKey="subject" tick={{ fontSize: 10 }} />
              <Radar name="Current Proficiency" dataKey="A" stroke="#7c3aed" fill="#7c3aed" fillOpacity={0.2} />
            </RadarChart>
          </ResponsiveContainer>
        </div>
        <div className="bg-white dark:bg-slate-900 rounded-xl border p-4 shadow-sm">
      ...
      {/* === SECTION 3: Skill Gap Bar Chart (New) === */}
      <div className="bg-white dark:bg-slate-900 rounded-xl border p-4 shadow-sm">
        <h3 className="text-sm font-semibold mb-3 flex items-center gap-2"><BarChart3 className="w-4 h-4 text-purple-500" /> Skill Gap Analysis (Current vs Target)</h3>
        <ResponsiveContainer width="100%" height={280}>
          <BarChart data={skillGapData} margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
            ...
            <Tooltip />
            <Legend />
            <Bar dataKey="current" name="Current" fill="#7c3aed" barSize={20} radius={[4, 4, 0, 0]} />
      ...
      {/* === SECTION 4: Certification Growth + Application Status (Two columns) === */}
      <div className="grid md:grid-cols-2 gap-5">
        <div className="bg-white dark:bg-slate-900 rounded-xl border p-4 shadow-sm">
          ...
          <ResponsiveContainer width="100%" height={250}>
            <AreaChart data={certsPerMonth}>
              ...
              <Tooltip />
              <Area type="monotone" dataKey="certs" name="Certifications Earned" stroke="#7c3aed" fill="#7c3aed" fillOpacity={0.2} />
            </AreaChart>
          </ResponsiveContainer>
        </div>
        <div className="bg-white dark:bg-slate-900 rounded-xl border p-4 shadow-sm">
          ...
          <div className="flex items-center justify-around">
            ...
            <div className="space-y-1">
              <div><span className="text-2xl font-bold text-purple-600">{appliedJobs}</span><span className="text-xs text-slate-400"> / {careerOps.length} applied</span></div>
      ...
      {/* === SECTION 5: Learning Activity & Job Match Distribution (Two columns) === */}
      <div className="grid md:grid-cols-2 gap-5">
        <div className="bg-white dark:bg-slate-900 rounded-xl border p-4 shadow-sm">
        ...
        <div className="bg-white dark:bg-slate-900 rounded-xl border p-4 shadow-sm">
          <h3 className="text-sm font-semibold mb-3 flex items-center gap-2"><Target className="w-4 h-4 text-purple-500" /> Job Match Distribution</h3>
          <ResponsiveContainer width="100%" height={250}>
            <BarChart data={matchRanges} margin={{ top: 5, right: 20, left: 0, bottom: 5 }}>
              ...
              <YAxis tick={{ fontSize: 10 }} />
              <Tooltip />
              <Bar dataKey="count" name="Number of Jobs" fill="#7c3aed" radius={[4, 4, 0, 0]} />
            </BarChart>
          </ResponsiveContainer>
        </div>
      </div>
      
      {/* === SECTION 6: Portfolio Engagement (Conditional) === */}
      {portfolio.length > 0 && (
        <div className="bg-white dark:bg-slate-900 rounded-xl border p-4 shadow-sm">
          <h3 className="text-sm font-semibold mb-3 flex items-center gap-2"><Globe className="w-4 h-4 text-purple-500" /> Portfolio Engagement (Views per Project)</h3>
          <ResponsiveContainer width="100%" height={280}>
